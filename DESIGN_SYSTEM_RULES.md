...
Used sparingly on cards and elevated surfaces. Prefer no shadow in most cases for the flat terminal aesthetic.
...
| `mode.color.bg.accentHover` | `bg-accent/90` | Accent hover state |
| `mode.color.bg.primarySubtle` | `bg-primary/5` | Very light primary tint |
...
| `mode.color.bg.successSubtle` | `bg-success/20` | Subtle success background |
| `mode.color.bg.warning` | `bg-warning` | Warning background |
| `mode.color.bg.warningMuted` | `bg-warning/10` | Warning tint |
| `mode.color.bg.info` | `bg-info` | Info background |
| `mode.color.bg.infoMuted` | `bg-info/10` | Info tint |
| `mode.color.bg.muted` | `bg-muted` | Muted/secondary backgrounds |
| `mode.color.bg.mutedSubtle` | `bg-muted/20` | Very light muted |
| `mode.color.bg.mutedLight` | `bg-muted/30` | Light muted |
| `mode.color.bg.mutedMedium` | `bg-muted/50` | Medium muted |
...
| `mode.color.text.infoOnColor` | `text-info-foreground` | Text on info bg |
...
| `mode.color.border.focus` | `border-ring` | Focused input border |
...
| `mode.color.border.accentSubtle` | `border-primary/30` | Subtle accent border |
| `mode.color.border.mutedSubtle` | `border-muted-foreground/30` | Muted border |

---

### `mode.color.icon.*`

Icon tint classes (same as text tokens but semantically named for icons).

| Property | Tailwind class |
|---|---|
| `mode.color.icon.primary` | `text-foreground` |
| `mode.color.icon.secondary` | `text-card-foreground` |
| `mode.color.icon.muted` | `text-muted-foreground` |
| `mode.color.icon.accent` | `text-accent` |
| `mode.color.icon.danger` | `text-destructive` |
| `mode.color.icon.success` | `text-success` |
| `mode.color.icon.warning` | `text-warning` |
| `mode.color.icon.info` | `text-info` |
...
| `mode.state.hover.cardSubtle` | `hover:bg-muted/30` | Subtle card hover |
| `mode.state.hover.link` | `hover:text-primary` | Link hover color |
| `mode.state.hover.linkOpacity` | `hover:opacity-80` | Link opacity hover |
| `mode.state.hover.listItem` | `hover:bg-muted/50` | List item hover |
| `mode.state.hover.opacity` | `hover:opacity-90` | Generic opacity hover |
| `mode.state.hover.borderWarning` | `hover:border-warning` | Warning border on hover |
| `mode.state.hover.textWarning` | `hover:text-warning` | Warning text on hover |
| `mode.state.hover.borderAccent` | `hover:border-primary` | Accent border on hover |
| `mode.state.hover.textAccent` | `hover:text-primary` | Accent text on hover |
...
| Property | Value | Use |
|---|---|---|
| `mode.state.completed.opacity` | `opacity-60` | Completed/done items |
...
M3 type scale tokens. Use these instead of arbitrary `text-*` sizes.

**Display (hero/marketing):**

| Property | Equivalent size |
|---|---|
| `mode.typography.display.xl` | `text-display-xl` (88px) |
| `mode.typography.display.l` | `text-display-l` (57px) |
| `mode.typography.display.m` | `text-display-m` (45px) |
| `mode.typography.display.s` | `text-display-s` (36px) |

**Headline (page titles):**

| Property | Equivalent size |
|---|---|
| `mode.typography.headline.l` | `text-headline-l` (32px) |
| `mode.typography.headline.m` | `text-headline-m` (28px) |
...
| Property | Equivalent size |
|---|---|
| `mode.typography.title.l` | `text-title-l` (22px) |
| `mode.typography.title.m` | `text-title-m` (16px) |
| `mode.typography.title.s` | `text-title-s` (14px) |

**Body (running text):**

| Property | Equivalent size |
|---|---|
| `mode.typography.body.l` | `text-body-l` (16px) |
| `mode.typography.body.m` | `text-body-m` (14px) |
| `mode.typography.body.s` | `text-body-s` (12px) |

**Label (UI components):**

| Property | Equivalent size |
|---|---|
| `mode.typography.label.l` | `text-label-l` (14px) |
| `mode.typography.label.m` | `text-label-m` (12px) |
| `mode.typography.label.s` | `text-label-s` (11px) |

**Code (terminal output):**

| Property | Equivalent size |
|---|---|
| `mode.typography.code.l` | `text-code-l` (16px) |
| `mode.typography.code.m` | `text-code-m` (14px) |
| `mode.typography.code.s` | `text-code-s` (12px) |

**Legacy aliases (backwards-compatible, still acceptable):**
...
Fixed-size utility classes for consistent layout dimensions.

| Property | Value | Use |
|---|---|---|
| `mode.sizing.panel` | `h-panel` (600px) | Full-height panels |
| `mode.sizing.panelSm` | `h-panel-sm` (400px) | Shorter panels |
| `mode.sizing.sidebar` | `w-sidebar` (288px) | Sidebar width |
| `mode.sizing.auth` | `max-w-auth` (400px) | Auth page max-width |
| `mode.sizing.dropdown` | `min-w-dropdown` (8rem) | Dropdown min-width |
| `mode.sizing.select` | `min-w-select` (5rem) | Select min-width |
| `mode.sizing.dropdownHeight` | `max-h-dropdown` (300px) | Dropdown max-height |
| `mode.sizing.textareaHeight` | `max-h-textarea` (200px) | Textarea max-height |
...
| Property | Value | Use |
|---|---|---|
| `mode.zIndex.banner` | `z-banner` (60) | Cookie consent, floating notices |
| `mode.zIndex.modal` | `z-modal` (100) | Modals, navigation, toast |
