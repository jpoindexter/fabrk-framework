...
const columns: ColumnDef<RepoRow>[] = [
  ...
  },
  {
    ...
    cell: ({ row }) => {
      ...
      return (
        <Badge
          variant={s === 'passing' ? 'default' : s === 'warning' ? 'outline' : 'destructive'}
        >
          {s.toUpperCase()}
        </Badge>
      )
...
const sidebarItems = [
  { id: 'overview', label: 'Overview', icon: <LayoutDashboard className="size-4" />, href: '/dashboard', active: true },
