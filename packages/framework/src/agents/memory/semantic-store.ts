...
export class SemanticMemoryStore implements MemoryStore {
  ...
  async appendMessage(
    ...
    if (msg.role === "user" || msg.role === "assistant") {
      ...
      this.embedAsync(message.id, message.content, thread?.agentName);
    }
  ...
  async search(
    ...
    // Collect messages — need to search across all threads
    const found: ThreadMessage[] = [];
    for (const msgId of messageIds) {
      // We stored the message content inline; search through base store
      // For in-memory, we can search all threads
      found.push({
        id: msgId,
        threadId: "",
        role: "user",
  ...
  private embedAsync(messageId: string, content: string, agentName?: string): void {
