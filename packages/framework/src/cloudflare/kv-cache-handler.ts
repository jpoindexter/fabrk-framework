/**
 * Cloudflare KV-backed CacheHandler for fabrk.
 *
 ...
 *
 *   import { KVCacheHandler } from "fabrk/cloudflare";
 *   import { setCacheHandler } from "fabrk/shims/cache";
 *
 *   export default {
...
export class KVCacheHandler implements CacheHandler {
  ...
  async get(
    ...
    if (!entry) {
      console.error("[fabrk] Invalid cache entry shape for key:", key);
      await this.kv.delete(kvKey);
      return null;
    }
...
function safeBase64ToArrayBuffer(base64: string): ArrayBuffer | null {
  try {
    ...
    console.error("[fabrk] Invalid base64 in cache entry");
    return null;
  }
}
