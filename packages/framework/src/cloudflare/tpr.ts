...
function extractFromJSON(config: Record<string, unknown>): WranglerConfig {
  ...
  if (Array.isArray(config.kv_namespaces)) {
    const fabrkKV = config.kv_namespaces.find(
      (ns: Record<string, unknown>) =>
        ns && typeof ns === "object" && ns.binding === "VINEXT_CACHE",
    );
    if (
      fabrkKV &&
      typeof fabrkKV.id === "string" &&
      fabrkKV.id !== "<your-kv-namespace-id>"
    ) {
      result.kvNamespaceId = fabrkKV.id;
    }
  }
...
function filterTrafficPaths(entries: TrafficEntry[]): TrafficEntry[] {
  return entries.filter((e) => {
    ...
    // Internal routes
    if (e.path.startsWith("/_fabrk/") || e.path.startsWith("/_next/")) return false;
    // RSC requests
...
/**
 * Spawn a subprocess running the fabrk production server.
 * Uses the same Node.js binary and resolves prod-server.js relative
 * to the current module (works whether fabrk is installed or linked).
 */
function startLocalServer(root: string, port: number): ChildProcess {
  ...
  const script = [
    ...
    `.catch(e => { console.error("[fabrk-tpr] Server failed to start:", e); process.exit(1); });`,
  ].join("");
...
async function uploadToKV(
  ...
  for (const [routePath, result] of entries) {
    ...
    const revalidateHeader = result.headers["x-fabrk-revalidate"];
 ...
 * the `fabrk deploy` pipeline. Gracefully skips (never errors) when
