    ...
    try {
      ...
      if (isCjsError(e) && (filename.endsWith(".js") || filename.endsWith(".cjs"))) {
        try {
          ...
          console.warn(
            `[fabrk] Failed to load ${filename}: ${(e2 as Error).message}`,
          );
          return null;
        }
      }
      ...
      console.warn(
        `[fabrk] Failed to load ${filename}: ${(e as Error).message}`,
      );
      return null;
    }
  }
...
export async function resolveNextConfig(
  ...
  for (const key of unsupported) {
    if (config[key] !== undefined) {
      console.warn(
        `[fabrk] next.config option "${key}" is not yet supported and will be ignored`,
      );
    }
  }
  ...
  if (output && output !== "export" && output !== "standalone") {
    console.warn(`[fabrk] Unknown output mode "${output}", ignoring`);
  }
