 ...
 *
 * Cache dir structure: .fabrk/fonts/<family-hash>/
...
const VIRTUAL_SERVER_ENTRY = "virtual:fabrk-server-entry";
const RESOLVED_SERVER_ENTRY = "\0" + VIRTUAL_SERVER_ENTRY;
const VIRTUAL_CLIENT_ENTRY = "virtual:fabrk-client-entry";
const RESOLVED_CLIENT_ENTRY = "\0" + VIRTUAL_CLIENT_ENTRY;

// Virtual module IDs for App Router entries
const VIRTUAL_RSC_ENTRY = "virtual:fabrk-rsc-entry";
const RESOLVED_RSC_ENTRY = "\0" + VIRTUAL_RSC_ENTRY;
const VIRTUAL_APP_SSR_ENTRY = "virtual:fabrk-app-ssr-entry";
const RESOLVED_APP_SSR_ENTRY = "\0" + VIRTUAL_APP_SSR_ENTRY;
const VIRTUAL_APP_BROWSER_ENTRY = "virtual:fabrk-app-browser-entry";
const RESOLVED_APP_BROWSER_ENTRY = "\0" + VIRTUAL_APP_BROWSER_ENTRY;

/** Image file extensions handled by the fabrk:image-imports plugin.
...
/** Absolute path to fabrk's shims directory, used by clientManualChunks. */
 ...
 * - "fabrk"    — fabrk shims (router, head, link, etc.)
 *
...
function clientManualChunks(id: string): string | undefined {
  ...
  // fabrk shims — small runtime, shared across all pages.
...
export interface FabrkOptions {
   ...
   *
   * By default, fabrk auto-detects: checks for app/ and pages/ at the
...
export default function fabrk(options: FabrkOptions = {}): Plugin[] {
  ...
  async function generateServerEntry(): Promise<string> {
    ...
    const fabrkConfigJson = JSON.stringify({
...
function __safeRegExp(pattern, flags) {
  if (!__isSafeRegex(pattern)) {
    console.warn("[fabrk] Ignoring potentially unsafe regex pattern (ReDoS risk): " + pattern);
    return null;
  }
...
export async function runMiddleware(request) {
  ...
  try { response = await middlewareFn(nextRequest); }
    ...
    console.error("[fabrk] Middleware error:", e);
...
import { withFetchCache } from "fabrk/fetch-cache";
import { safeJsonStringify } from "fabrk/html";
...
export const fabrkConfig = ${fabrkConfigJson};
...
function triggerBackgroundRegeneration(key, renderFn) {
  ...
  const promise = renderFn()
    .catch((err) => console.error("[fabrk] ISR regen failed for " + key + ":", err))
...
function collectAssetTags(manifest, moduleIds) {
  // Fall back to embedded manifest (set by fabrk:cloudflare-build for Workers)
  ...
  // Inject the client entry script if embedded by fabrk:cloudflare-build
  ...
  if (m) {
    // Always inject shared chunks (framework, fabrk runtime, entry) and
    ...
    if (moduleIds && moduleIds.length > 0) {
      ...
      // fabrk runtime, and the entry bootstrap. These are identified
      ...
      for (var key in m) {
        ...
        for (var vi = 0; vi < vals.length; vi++) {
          ...
          if (
            ...
            basename.startsWith("fabrk-") ||
            basename.includes("fabrk-client-entry") ||
            basename.includes("fabrk-app-browser-entry")
          ) {
            allFiles.push(file);
          }
        }
...
export async function renderPage(request, url, manifest) {
  ...
  try {
    ...
    try {
      ...
      if (allFontStyles.length > 0) { fontHeadHTML += '<style data-fabrk-fonts>' + allFontStyles.join("\\n") + '</style>\\n  '; }
    ...
    console.error("[fabrk] SSR error:", e);
...
export async function handleApiRoute(request, url) {
  ...
  try {
    ...
    console.error("[fabrk] API error:", e);
    return new Response("Internal Server Error", { status: 500 });
  }
}
...
async function hydrate() {
  ...
  if (!nextData) {
    console.error("[fabrk] No __NEXT_DATA__ found");
    return;
  }
  ...
  if (!loader) {
    console.error("[fabrk] No page loader for route:", nextData.page);
    return;
  }
  ...
  const PageComponent = pageModule.default;
  if (!PageComponent) {
    console.error("[fabrk] Page module has no default export");
    return;
  }
  ...
  if (!container) {
    console.error("[fabrk] No #__next element found");
    return;
  }
...
`;
  ...
  // project root, not from fabrk's own package location. When fabrk is
  // installed via symlink (npm file: deps, pnpm workspace:*), a bare
  // import() resolves from fabrk's realpath, which can find a different
  ...
  if (earlyAppDirExists && autoRsc) {
    if (!resolvedRscPath) {
      throw new Error(
        "fabrk: App Router detected but @vitejs/plugin-rsc is not installed.\n" +
  ...
  const plugins: (Plugin | Promise<Plugin[]>)[] = [
    ...
    tsconfigPaths(),
    {
      ...
      async config(config) {
        ...
        nextShimMap = {
          ...
          "fabrk/error-boundary": path.join(shimsDir, "error-boundary"),
          "fabrk/layout-segment-context": path.join(shimsDir, "layout-segment-context"),
          "fabrk/metadata": path.join(shimsDir, "metadata"),
          "fabrk/fetch-cache": path.join(shimsDir, "fetch-cache"),
          "fabrk/cache-runtime": path.join(shimsDir, "cache-runtime"),
          "fabrk/navigation-state": path.join(shimsDir, "navigation-state"),
          "fabrk/router-state": path.join(shimsDir, "router-state"),
          "fabrk/head-state": path.join(shimsDir, "head-state"),
          "fabrk/instrumentation": path.resolve(__dirname, "server", "instrumentation"),
          "fabrk/html": path.resolve(__dirname, "server", "html"),
        };
        ...
        if (!hasMdxPlugin && hasMdxFiles(root, hasAppDir ? appDir : null, hasPagesDir ? pagesDir : null)) {
          try {
            ...
            if (nextConfig.mdx) {
              console.log("[fabrk] Auto-injected @mdx-js/rollup with remark/rehype plugins from next.config");
            } else {
              console.log("[fabrk] Auto-injected @mdx-js/rollup for MDX support");
            }
          } catch {
            // @mdx-js/rollup not installed — warn but don't fail
            console.warn(
              "[fabrk] MDX files detected but @mdx-js/rollup is not installed. " +
        ...
        const viteConfig: Record<string, any> = {
          ...
          build: {
            rollupOptions: {
              ...
              // fabrk runtime (shims), and vendor packages into their own
          ...
          // OPTIONS with a 204 before the request reaches fabrk's handler.
          ...
          resolve: {
            ...
            // When fabrk is linked (npm link / bun link) or any dependency
          ...
          // Exclude fabrk from dependency optimization so esbuild doesn't
          // scan dist files containing virtual module imports (virtual:fabrk-*)
          ...
          viteConfig.environments = {
            ...
            client: {
              ...
              build: {
                ...
                // so the fabrk:cloudflare-build closeBundle hook can read the
      ...
      configResolved(config) {
        // Detect double RSC plugin registration. When fabrk auto-injects
        ...
        if (rscPluginPromise) {
          ...
          if (rscRootPlugins.length > 1) {
            throw new Error(
              "[fabrk] Duplicate @vitejs/plugin-rsc detected.\n" +
              "         fabrk auto-registers @vitejs/plugin-rsc when app/ is detected.\n" +
              "         Your config also registers it manually, which doubles build time.\n\n" +
              "         Fix: remove the explicit rsc() call from your plugins array.\n" +
              "         Or: pass rsc: false to fabrk() if you want to configure rsc() yourself.",
            );
          }
        }
      },
      ...
      resolveId: {
        // Hook filter: only invoke JS for next/* imports and virtual:fabrk-* modules.
        // Matches "next/navigation", "next/router.js", "virtual:fabrk-rsc-entry",
        // and \0-prefixed re-imports from @vitejs/plugin-rsc.
        filter: {
          id: /(?:next\/|virtual:fabrk-)/,
        },
        handler(id) {
    ...
    {
      name: "fabrk:react-canary",
      enforce: "pre",

      resolveId(id) {
        if (id === "virtual:fabrk-react-canary") return "\0virtual:fabrk-react-canary";
      },

      load(id) {
        if (id === "\0virtual:fabrk-react-canary") {
          return [
            `export * from "react";`,
      ...
      transform(code, id) {
        ...
        const result = code.replace(
          /from\s*['"]react['"]/g,
          'from "virtual:fabrk-react-canary"',
        );
        if (result !== code) {
      ...
      configureServer(server: ViteDevServer) {
        ...
        if (instrumentationPath) {
          runInstrumentation(server, instrumentationPath).catch((err) => {
            console.error("[fabrk] Instrumentation error:", err);
          });
        }
            ...
            try {
              ...
              if (middlewarePath) {
                ...
                if (result.rewriteStatus) {
                  (req as any).__fabrkRewriteStatus = result.rewriteStatus;
                }
              }
              ...
              const mwStatus = (req as any).__fabrkRewriteStatus as number | undefined;
    ...
    // adding a `?fabrk-meta` suffix: the original import gets the URL from Vite,
    // and we resolve the `?fabrk-meta` virtual module to provide dimensions.
    {
      name: "fabrk:image-imports",
      enforce: "pre",
      ...
      resolveId: {
        filter: { id: /\?fabrk-meta$/ },
        handler(source, _importer) {
          if (!source.endsWith("?fabrk-meta")) return null;
          // Resolve the real image path from the importer
          const realPath = source.replace("?fabrk-meta", "");
          return `\0fabrk-image-meta:${realPath}`;
        },
      },

      async load(id) {
        if (!id.startsWith("\0fabrk-image-meta:")) return null;
        const imagePath = id.replace("\0fabrk-image-meta:", "");
      ...
      transform: {
        ...
        async handler(code, id) {
          ...
          while ((match = imageImportRe.exec(code)) !== null) {
            ...
            const urlVar = `__fabrk_img_url_${varName}`;
            const metaVar = `__fabrk_img_meta_${varName}`;
            const replacement =
              `import ${urlVar} from ${JSON.stringify(importPath)};\n` +
              `import ${metaVar} from ${JSON.stringify(absImagePath + "?fabrk-meta")};\n` +
    ...
    // caches them locally in .fabrk/fonts/, and rewrites font constructor
    ...
    // In dev mode, this plugin is a no-op (CDN loading is used instead).
    {
      ...
      configResolved(config) {
        ...
        (this as any)._cacheDir = path.join(config.root, ".fabrk", "fonts");
      },
    ...
    // URLs are correct.
    {
      ...
      transform: {
        ...
        handler(code, id) {
          ...
          // Skip fabrk's own font-local shim — it contains example paths
          ...
          while ((match = fontPathRe.exec(code)) !== null) {
            ...
            const varName = `__fabrk_local_font_${fontImportCounter++}`;
            ...
            // Replace: path: "./font.woff2" -> path: __fabrk_local_font_0
            const matchStart = match.index;
            const matchEnd = matchStart + fullMatch.length;
    ...
    // exports/functions with registerCachedFunction() from fabrk/cache-runtime.
    // Runs without enforce so it executes after JSX transform (parseAst needs plain JS).
    {
      name: "fabrk:use-cache",
      ...
      transform: {
        ...
        async handler(code, id) {
          ...
          if (!resolvedRscTransformsPath) {
            throw new Error(
              "fabrk: 'use cache' requires @vitejs/plugin-rsc to be installed.\n" +
              "Run: npm install -D @vitejs/plugin-rsc",
            );
          }
          ...
          if (cacheDirective) {
            ...
            if (result.exportNames.length > 0) {
              ...
              output.overwrite(cacheDirective.start, cacheDirective.end, `/* "use cache" — wrapped by fabrk */`);
              return {
            ...
            output.overwrite(cacheDirective.start, cacheDirective.end, `/* "use cache" — handled by fabrk */`);
            return {
              code: output.toString(),
              map: output.generateMap({ hires: "boundary" }),
            };
      ...
      closeBundle: {
        ...
        async handler() {
          ...
          if (hasAppDir) {
            ...
            // Pages Router uses "fabrk-client-entry", App Router uses
            // "fabrk-app-browser-entry".
            if (!clientEntryFile) {
              const assetsDir = path.join(clientDir, "assets");
              if (fs.existsSync(assetsDir)) {
                const files = fs.readdirSync(assetsDir);
                const entry = files.find((f: string) =>
                  (f.includes("fabrk-client-entry") || f.includes("fabrk-app-browser-entry")) && f.endsWith(".js"));
          ...
          if (!fs.existsSync(headersPath)) {
            ...
            const headersContent = [
              "# Cache content-hashed assets immutably (generated by fabrk)",
              `/${assetsDir}/*`,
...
async function proxyExternalRewriteNode(
  ...
  try {
    ...
    console.error("[fabrk] External rewrite proxy error:", e);
    if (!res.headersSent) {
      res.writeHead(502);
      res.end("Bad Gateway");
    }
