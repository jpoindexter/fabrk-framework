...
import { ErrorBoundary, NotFoundBoundary } from "fabrk/error-boundary";
import { LayoutSegmentProvider } from "fabrk/layout-segment-context";
import { MetadataHead, mergeMetadata, resolveModuleMetadata, ViewportHead, mergeViewport, resolveModuleViewport } from "fabrk/metadata";
...
import { runWithFetchCache } from "fabrk/fetch-cache";
import { clearPrivateCache as _clearPrivateCache } from "fabrk/cache-runtime";
// Import server-only state module to register ALS-backed accessors.
import "fabrk/navigation-state";
import { reportRequestError as _reportRequestError } from "fabrk/instrumentation";
...
function __validateCsrfOrigin(request) {
  ...
  console.warn(
    \`[fabrk] CSRF origin mismatch: origin "\${originHost}" does not match host "\${hostHeader}". Blocking server action request.\`
  );
  return new Response("Forbidden", { status: 403, headers: { "Content-Type": "text/plain" } });
}
...
async function __proxyExternalRequest(request, externalUrl) {
  ...
  catch (e) { console.error("[fabrk] External rewrite proxy error:", e); return new Response("Bad Gateway", { status: 502 }); }
...
async function _handleRequest(request) {
  ...
  if (route.routeHandler) {
    ...
    if (typeof handlerFn === "function") {
      try {
        ...
        setHeadersContext(null);
        setNavigationContext(null);
        console.error("[fabrk] Route handler error:", err);
        _reportRequestError(
          err instanceof Error ? err : new Error(String(err)),
          { path: cleanPathname, method: request.method, headers: Object.fromEntries(request.headers.entries()) },
  ...
  if (dynamicParamsConfig === false && route.isDynamic && typeof route.page?.generateStaticParams === "function") {
    try {
      ...
      console.error("[fabrk] generateStaticParams error:", err);
    }
  }
  ...
  async function pumpReader() {
    ...
    try {
      ...
      if (process.env.NODE_ENV !== "production") {
        console.warn("[fabrk] RSC embed stream read error:", err);
      }
      done = true;
    }
    reading = false;
  ...
  try {
    ...
    if (fontData) {
      ...
      if (fontData.styles && fontData.styles.length > 0) {
        fontHTML += '<style data-fabrk-fonts>' + fontData.styles.join("\\n") + '</style>\\n';
      }
    }
...
async function main() {
  ...
  window.__VINEXT_RSC_NAVIGATE__ = async function navigateRsc(href) {
    try {
      ...
      console.error("[fabrk] RSC navigation error:", err);
  ...
  if (import.meta.hot) {
    import.meta.hot.on("rsc:update", async () => {
      try {
        ...
        console.error("[fabrk] RSC HMR error:", err);
      }
    });
  }
}
