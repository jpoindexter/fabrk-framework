 ...
 * 2. **Self-hosted mode** (production build): The fabrk:google-fonts Vite plugin
...
function injectClassNameRule(
  ...
  style.setAttribute("data-fabrk-font-class", className);
...
function injectVariableClassRule(
  ...
  const style = document.createElement("style");
  style.textContent = css;
  style.setAttribute("data-fabrk-font-variable", variableClassName);
  document.head.appendChild(style);
}
...
function injectSelfHostedCSS(css: string): void {
  ...
  const style = document.createElement("style");
  style.textContent = css;
  style.setAttribute("data-fabrk-font-selfhosted", "true");
  document.head.appendChild(style);
}
