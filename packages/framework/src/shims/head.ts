...
function reactElementToHTML(child: React.ReactElement): string {
  ...
  if (!ALLOWED_HEAD_TAGS.has(tag)) {
    if (process.env.NODE_ENV !== "production") {
      console.warn(
        `[fabrk] <Head> ignoring disallowed tag <${tag}>. ` +
  ...
  if (selfClosing.includes(tag)) {
    return `<${tag}${attrStr} data-fabrk-head="true" />`;
  }

  return `<${tag}${attrStr} data-fabrk-head="true">${innerHTML}</${tag}>`;
}
...
function Head({ children }: HeadProps): null {
  ...
  useEffect(() => {
    ...
    // Remove previous fabrk-managed head elements
    document
      .querySelectorAll("[data-fabrk-head]")
    ...
    Children.forEach(children, (child) => {
      ...
      domEl.setAttribute("data-fabrk-head", "true");
