...
function saveScrollPosition(): void {
  ...
  window.history.replaceState(
    { ...state, __fabrk_scrollX: window.scrollX, __fabrk_scrollY: window.scrollY },
    "",
  );
}
...
function restoreScrollPosition(state: unknown): void {
  if (state && typeof state === "object" && "__fabrk_scrollY" in state) {
    const { __fabrk_scrollX: x, __fabrk_scrollY: y } = state as {
      __fabrk_scrollX: number;
      __fabrk_scrollY: number;
    };
...
async function navigateClient(url: string): Promise<void> {
  ...
  try {
    ...
    // Get the page module URL from __NEXT_DATA__.__fabrk (preferred),
    // or fall back to parsing the hydration script
    let pageModuleUrl: string | undefined =
      nextData.__fabrk?.pageModuleUrl;
    ...
    console.error("[fabrk] Client navigation failed:", err);
...
export function useRouter(): NextRouter {
  ...
  useEffect(() => {
    ...
    window.addEventListener("fabrk:navigate", onNavigate);
    return () => window.removeEventListener("fabrk:navigate", onNavigate);
  }, []);
  ...
  const push = useCallback(
    async (url: string | UrlObject, _as?: string, options?: TransitionOptions): Promise<boolean> => {
      ...
      if (isHashOnlyChange(resolved)) {
        ...
        scrollToHash(hash);
        setState(getPathnameAndQuery());
        window.dispatchEvent(new CustomEvent("fabrk:navigate"));
        return true;
      }
