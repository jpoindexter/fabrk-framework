/**
 * @fabrk/ui — Component Registry
 *
 * Components are COPIED into your project (like shadcn/ui), not imported as npm deps.
 *
 * Usage:
 * ```bash
 * fabrk add button card dialog     # Copy into src/components/ui/
 * fabrk add bar-chart sparkline    # Copy chart components
 * fabrk add chat                   # Copy AI chat components
 * ```
 *
 * This package exports the registry metadata and resolution utilities
 * used by the `fabrk` CLI to discover and copy components.
 */
