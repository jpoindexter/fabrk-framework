# FABRK AI SaaS Template

A complete AI-powered SaaS application built with [FABRK Framework](https://github.com/jpoindexter/fabrk-framework).

## Getting Started

1. **Install dependencies:**
   ```bash
   pnpm install
   ```

2. **Run development server:**
   ```bash
   pnpm dev
   ```

3. **Open your browser:**
   Navigate to [http://localhost:3000](http://localhost:3000)

## What's Included

- ✅ **Next.js 15** with App Router
- ✅ **70+ UI Components** from `@fabrk/components`
- ✅ **AI Toolkit** with cost tracking and validation
- ✅ **Terminal Design System** with 18 themes
- ✅ **TypeScript** for type safety
- ✅ **Tailwind CSS** for styling
- ✅ **React 19** with latest features

## AI Features

- 💰 **Cost Tracking** - Real-time AI spending monitoring
- ✅ **Validation** - Type-safe request/response validation
- 🔌 **Multiple Providers** - OpenAI, Anthropic, Google support
- 📊 **Analytics** - Per-feature cost breakdown

## Project Structure

```
├── app/              # Next.js app router
│   ├── layout.tsx    # Root layout with FABRK providers
│   └── page.tsx      # Home page
├── public/           # Static assets
└── package.json      # Dependencies
```

## Available Scripts

- `pnpm dev` - Start development server
- `pnpm build` - Build for production
- `pnpm start` - Start production server
- `pnpm lint` - Run ESLint
- `pnpm type-check` - Run TypeScript checks

## Learn More

- [FABRK Documentation](https://github.com/jpoindexter/fabrk-framework)
- [Next.js Documentation](https://nextjs.org/docs)
- [Component Examples](https://github.com/jpoindexter/fabrk-framework/tree/main/packages/components)

## Deploy

The easiest way to deploy is with [Vercel](https://vercel.com):

[![Deploy with Vercel](https://vercel.com/button)](https://vercel.com/new)
